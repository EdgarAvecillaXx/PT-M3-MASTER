...
function problemB() {
   ...
   */
  // promise version
  Promise.map(filenames, filename => promisifiedReadFile(filename).then(stanza => blue(stanza))).then(() => {
    console.log('done');
  });
}
function problemC() {
  ...
  Promise.reduce(
    filenames,
    (total, filename) => promisifiedReadFile(filename).then(stanza => blue(stanza)),
    0
...
function problemE() {
  ...
  function promisifiedWriteFile(filename, str) {
    fs.writeFile(filename, str, function (err) {
      if (err) reject(err);
      else resolve('Escritura existosa');
    });
  }
}
