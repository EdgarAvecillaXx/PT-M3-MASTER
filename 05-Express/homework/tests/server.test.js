...
describe('Request', () => {
  ...
  describe(`${METHOD_POST} ${PATH}/author/:author`, () => {
    it('Agrega un nuevo Post', () => {
      ...
      req(METHOD_POST, STATUS_OK, post, `${PATH}/author/Mariana`).then(res => {
  ...
  describe(`${METHOD_GET} ${PATH}`, () => {
    it('Obtiene el listado de Posts', () => {
      return req(METHOD_GET, STATUS_OK).then(posts => expect(posts).to.have.length(0));
    });
  ...
  describe(`${METHOD_PUT} ${PATH}`, () => {
    ...
    it('Informa que falta el parámetro `title`', () => {
      return addPost({ author: 'author', title: 'title', contents: 'contents' }).then(post => {
